<div class="modal-content">
    <form
        id="formAction"
        action="{{ $school->id ? route('schools.update',$school->id) : route('schools.store') }}"
        method="post"
        >
        @csrf
        @if ($school->id)
        ...
        <div class="modal-header">
            <h5 class="modal-title" id="largeModalLabel">
                {{ $school->id ? 'Edit' : 'Tambah' }} {{ ucFirst(request()->segment(2)) }}
            </h5>
            <button
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select id="headmaster_id" class="form-control" name="headmaster_id">
                            <option value="">-- Pilih nama Kepala --</option>
                            @foreach ($kepsek as $user)
                                <option
                                    value="{{ $user->id }}"
                                    @selected($user->id == $school->headmaster_id)
                                    >
                    ...
                    <div class="mb-3">
                        ...
                        <select id="coordinator_id" class="form-control" name="coordinator_id">
                            ...
                            @foreach ($korgur as $user)
                                <option
                                    value="{{ $user->id }}"
                                    @selected($user->id == $school->coordinator_id)
                                    >
                                    {{ Str::ucfirst($user->name) }}
                                </option>
                            @endforeach
