                    ...
                    <div class="card-body">
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
                            <div>
                                ...
                                <div class="text-muted small">Upload file Excel, review preview data, lalu commit hanya baris baru yang dipilih.</div>
                            </div>
                            <div class="d-flex flex-wrap gap-2">
                                <a href="{{ route('users.index') }}" class="btn btn-outline-secondary btn-sm">Kembali ke Users</a>
                                <a href="{{ route('users.importtemplate', ['role' => $role]) }}" class="btn btn-outline-primary btn-sm">Download Template {{ ucfirst($role) }}</a>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('users.importpreview', ['role' => $role]) }}" method="POST" enctype="multipart/form-data" class="row g-3 align-items-end mb-3">
                            @csrf
                            <div class="col-md-8">
                ...
                @if ($draft)
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
                                <div>
                                    <h5 class="mb-1">Preview Import {{ ucfirst($role) }}</h5>
                            ...
                            <form action="{{ route('users.importcommit', ['role' => $role]) }}" method="POST" id="commitImportForm">
                                @csrf

                                <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
                                    <div class="small text-muted">Centang hanya data dengan status <strong>Baru</strong> untuk dicommit.</div>
                                    <button type="submit" class="btn btn-success btn-sm" id="commitImportButton" disabled>Commit Data Terpilih</button>
                                </div>

                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered w-100" id="user-import-preview-table">
                                        <thead>
                                            <tr>
                                                <th style="width: 40px;">
                                                    <input type="checkbox" id="selectAllPreviewRows">
                                                </th>
                                                <th>Baris</th>
                                                <th>Status</th>
                                                <th>Username</th>
                                        ...
                                        <tbody>
                                            @foreach ($previewRows as $row)
                                                <tr class="{{ $row['is_selectable'] ? 'table-success' : '' }}">
                                                    ...
                                                    <td>{{ $row['name'] }}</td>
                                                    <td>{{ $row['email'] }}</td>
                                                    <td>{{ $row['password_preview'] }}</td>
                                                    <td>{{ $row['subject_id'] }} - {{ $row['subject_name'] }}</td>
                                                    <td>
                                                        @if ($row['notes'] === [])
                                                            <span class="text-success small">Data baru siap dicommit.</span>
                                                        @else
...
@push('js')
    ...
    <script>
        $(function() {
            const previewTable = $('#user-import-preview-table')
