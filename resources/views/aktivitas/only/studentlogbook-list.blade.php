                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table small-font table-striped table-hover table-sm">
                                ...
                                <tbody>
                                    @php
                                        $maps = App\Models\Map::where('lecture_id',auth()->user()->id)->where('year',$activeYear)
                                                                ->get()
                                    @endphp
                                    @forelse ($maps as $map)
                                    <tr>
                                        <td class="text-end">
                                            <a href="{{ route('diaryverifications.only.show',['map_id'=>$map->id]) }}" class="btn btn-sm btn-success">Logbook</a>
                                        </td>
                                        <td>
                                            {{ $map->students->name ?? '' }}
                                        </td>
