@push('css')
    ...
    <style>
        ...
        .lec-school-card-complete {
            border-color: rgba(18, 163, 111, 0.28);
        }

        .lec-school-card-incomplete {
        ...
        .lec-form-badge-done .lec-form-icon {
            color: #12a36f;
        }

        .lec-form-badge-pending {
            background: rgba(220, 53, 69, 0.1);
            color: #8a1f2c;
            border-color: rgba(220, 53, 69, 0.28);
        }

        .lec-form-badge-pending .lec-form-icon {
            color: #dc3545;
        }
        ...
        body.dark .lec-card-header {
            border-bottom-color: rgba(157, 185, 224, 0.18);
            background: linear-gradient(135deg, rgba(34, 49, 73, 0.88), rgba(25, 39, 61, 0.88));
        }

        body.dark .lec-card-header h5,
        body.dark .lec-card-header h6,
        body.dark .lec-identity-name,
        body.dark .lec-school-name,
        body.dark .lec-student-name {
            color: #e2ecff;
        }

        body.dark .lec-card-subtitle,
        body.dark .lec-identity-kicker,
        body.dark .lec-identity-meta {
            color: #abc0e0;
        }

        body.dark .lec-identity-badge,
        body.dark .lec-school-count {
            background: rgba(36, 118, 243, 0.2);
            color: #cfe1ff;
            border-color: rgba(36, 118, 243, 0.3);
        }

        body.dark .lec-school-card {
            border-color: rgba(157, 185, 224, 0.2);
            background: rgba(24, 37, 57, 0.82);
        }

        body.dark .lec-school-card-complete {
            border-color: rgba(18, 163, 111, 0.36);
        }

        body.dark .lec-school-card-incomplete {
            border-color: rgba(220, 53, 69, 0.28);
        }

        body.dark .lec-school-head {
            background: rgba(34, 49, 73, 0.6);
            border-bottom-color: rgba(157, 185, 224, 0.14);
        }

        body.dark .lec-school-progress {
            background: rgba(20, 31, 48, 0.5);
            border-bottom-color: rgba(157, 185, 224, 0.09);
        }

        body.dark .lec-student-row {
            border-bottom-color: rgba(157, 185, 224, 0.09);
        }

        body.dark .lec-progress-copy {
            color: #abc0e0;
        }

        body.dark .lec-progress-chip-complete {
            background: rgba(18, 163, 111, 0.2);
            color: #a0e8cc;
            border-color: rgba(18, 163, 111, 0.38);
        }

        body.dark .lec-progress-chip-incomplete {
            background: rgba(220, 53, 69, 0.18);
            color: #ffc0c8;
            border-color: rgba(220, 53, 69, 0.32);
        }

        body.dark .lec-form-badge-done {
            background: rgba(18, 163, 111, 0.2);
            color: #a0e8cc;
            border-color: rgba(18, 163, 111, 0.4);
        }

        body.dark .lec-form-badge-done .lec-form-icon {
            color: #3ee8a8;
        }

        body.dark .lec-form-badge-pending {
            background: rgba(220, 53, 69, 0.18);
            color: #ffc0c8;
            border-color: rgba(220, 53, 69, 0.34);
        }

        body.dark .lec-form-badge-pending .lec-form-icon {
            color: #ff8091;
        }
        ...
        body.dark .lec-student-progress-incomplete {
...
@php
    $lectureSchoolGroups = $lectureMaps->groupBy(fn($map) => $map->schools->name ?? 'Sekolah Belum Diatur');
@endphp
        ...
        <div class="lec-top">
            ...
            <div class="lec-card">
                <div class="lec-card-header">
                    <h6>Identitas DPL</h6>
                    <p class="lec-card-subtitle">Informasi akun dan akses cepat aktivitas penilaian.</p>
                </div>
                <div class="lec-card-body">
                    <p class="lec-identity-kicker">Dosen Pembimbing Lapangan</p>
                    <p class="lec-identity-name">{{ auth()->user()->name ?? '-' }}</p>
            ...
            <div class="lec-card">
                <div class="lec-card-header">
                    <h6>Ringkasan Mahasiswa Bimbingan</h6>
                    <p class="lec-card-subtitle">{{ $lectureMaps->count() }} mahasiswa di {{ $lectureSchoolGroups->count() }} sekolah.</p>
                </div>
                <div class="lec-card-body">
                    @if ($lectureMaps->isEmpty())
                        <div class="lec-empty">Belum ada mahasiswa yang dipetakan ke Anda pada tahun aktif ini.</div>
                    @else
                        <div class="lec-school-grid">
                            @foreach ($lectureSchoolGroups as $schoolName => $schoolMaps)
                                @php
                                ...
                                @endphp
                                <div class="lec-school-card lec-school-card-{{ $schoolIsComplete ? 'complete' : 'incomplete' }}">
                                    <div class="lec-school-head">
                                        <p class="lec-school-name">{{ $schoolName }}</p>
                                        <span class="lec-school-count">{{ $schoolMaps->count() }} mhs</span>
                                    </div>
                                    <div class="lec-school-progress">
                                        <p class="lec-progress-copy">Progres sekolah {{ $schoolCompletedForms }}/{{ $schoolTotalForms }} form</p>
                                        <span class="lec-progress-chip lec-progress-chip-{{ $schoolIsComplete ? 'complete' : 'incomplete' }}">
                                            ...
                                            <div class="lec-student-row">
                                                <div class="lec-student-info">
                                                    @if (isset($map->students->phone))
                                                        <a href="{{ 'http://wa.me/62'.$map->students->phone }}" target="_blank" class="lec-wa-btn"><i class="fa fa-whatsapp"></i></a>
                                                    @endif
                                                    <div class="lec-student-meta">
                                                        <p class="lec-student-name">{{ $map->students->name ?? '-' }}</p>
                                                        <span class="lec-student-progress lec-student-progress-{{ $studentIsComplete ? 'complete' : 'incomplete' }}">
