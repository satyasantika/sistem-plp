<script>
    var userRolePermission = function(table) {
        const modalSelector = '#modalAction';
        const modalDialogSelector = '#modalAction .modal-dialog';
        const tableSelector = `#${table}`;
        const eventNs = (table || 'rolepermission').replace(/[^a-zA-Z0-9]/g, '') || 'rolepermission';
        ...
        function bindStore() {
            $(document)
                ...
                .on(`submit.userRolePermissionStore.${eventNs}`, '#formAction', function(e) {
                    e.preventDefault();
        ...
        $(document)
            .off(`click.rolePermissionAction.${eventNs}`, `${tableSelector} .rolepermission-action`)
            .on(`click.rolePermissionAction.${eventNs}`, `${tableSelector} .rolepermission-action`, function() {
                let data = $(this).data();
                let id = data.id;
                let jenis = data.jenis;

                if (jenis == 'rolepermission') {
                    loadModalContent(`/konfigurasi/rolepermissions/${id}/edit`, 'Modal role permission tidak dapat dimuat.');
                    return;
                }

                if (jenis == 'userpermission') {
                    loadModalContent(`/konfigurasi/userpermissions/${id}/edit`, 'Modal user permission tidak dapat dimuat.');
                    return;
                }

                loadModalContent(`/konfigurasi/userroles/${id}/edit`, 'Modal user role tidak dapat dimuat.');
            });

        $(document)
            .off(`click.userRolePermissionModalDismiss.${eventNs}`, `${modalSelector} [data-bs-dismiss="modal"]`)
            .on(`click.userRolePermissionModalDismiss.${eventNs}`, `${modalSelector} [data-bs-dismiss="modal"]`, function(e) {
                e.preventDefault();
                closeModal();
            });
        ...
        $(document)
            ...
            .on(`click.userRolePermissionModalBackdrop.${eventNs}`, modalSelector, function(e) {
                if (e.target === this) {
                    closeModal();
                }
            });
